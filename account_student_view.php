    ...
    <div class="main-container">
        <div class="sidebar">
            ...
            <div class='type-2 d-none'>
                ...
                <a href="./students_management.php" class="field-tag">
                    <div class="field-group">
                        <i class="fa-solid fa-user-graduate" style="color: #ffc107;"></i>
                        <div class="title-field active-tag">Students</div>
                    </div>
                </a>
        ...
        <div class="main-content view-info-account-management ">
            <div class="info d-flex justify-content-between">
                ...
                <div class="image-view-info ">
                    ...
                    if ($imageID == "0" || $imageID == "") {
                        ...
                        $imgPath = getDataFromTheSameTable('imgPath', 'account_info_image', 'accountID', "$currentID", $conn);
            ...
            <div class="row header-table">
                ...
                <div class="col col-2">CourseName</div>
                <div class="col col-3">Class</div>
                <div class="col col-3">Lecturer</div>
                <div class="col col-3">Duration</div>
            </div>
            <?
                $data = $conn->query("SELECT * from class_student_list WHERE studentID = '$ID'");
                while($row = $data->fetch_assoc()){
                    $classID = $row['classID'];
                    $courseID = getDataFromTheSameTable('courseID','class','ID',$classID,$conn);
                    $courseName = getDataFromTheSameTable('courseName','course','ID',$courseID,$conn);
                    $className = getDataFromTheSameTable('className','class','ID',$classID,$conn);
                    $taughtByID = getDataFromTheSameTable('taughtBy','class','ID',$classID,$conn);
                    $startDate = getDataFromTheSameTable('startDate','class','ID',$classID,$conn);
                    $endDate = getDataFromTheSameTable('endDate','class','ID',$classID,$conn);
                    $fname = getDataFromTheSameTable('firstName','account_info','accountID',$taughtByID,$conn);
                    $lname = getDataFromTheSameTable('lastName','account_info','accountID',$taughtByID,$conn);
                    $shift = getDataFromTheSameTable("studyAt","class","ID",$classID,$conn);
                    $startWorking = "";
                    if($shift == 1){
                        $startWorking = $morning_working_hour;
                    } else $startWorking = $afternoon_working_hour;
                    echo "<div class='row'>
                    <div class='col col-1'>$classID</div>
                    <div class='col col-2'>$courseName</div>
                    <div class='col col-3'>$className</div>
                    <div class='col col-3'>$fname $lname</div>
                    <div class='col col-3'>From $startDate To $endDate<br>Shift: $shift - From: $startWorking</div>
                </div>";
                }
            ?>
        </div>
