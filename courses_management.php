...
<body>
        ...
        <div class="main-content">
            <div class="data-query">
                <div class="row header-table">
                    ...
                    <div class="col col-1">No. Lessons</div>
                    <div class="col col-2">No. Studying Hour</div>
                    <div class="col col-2">Created By</div>
                    <div class="col col-2">Action</div>
                </div>
                <div class="changable-data">
                    <?
                    $data = $conn->query("SELECT * FROM course where approveStatus ='approved'");
                    while($row = $data->fetch_assoc()){
                        $createdByID = $row['createdBy'];
                        $firstName = getDataFromTheSameTable('firstName', 'account_info', 'accountID' ,$createdByID, $conn);
                        $lastName = getDataFromTheSameTable('lastName', 'account_info', 'accountID' ,$createdByID, $conn);
                        $languageID = $row['languageID'];
                        $language = getDataFromTheSameTable('languageName','language','ID',$languageID,$conn);
                        $noLesson = ceil($row['numberofStudyingHour'] / $row['numberofStudyingHourEachLesson']);
                        echo ("<div class='row'>
                        <div class='col col-1'>$row[ID]</div>
                        <div class='col col-3'>$row[courseName]</div>
                        <div class='col col-1'>$language</div>
                        <div class='col col-1 text-center'>$noLesson</div>
                        <div class='col col-2 text-center'>$row[numberofStudyingHour]:00:00</div>
                        <div class='col col-2'>$firstName $lastName - ID: $row[createdBy]</div>
                        <div class='col col-2 text-center'>
                        <button class='btn btn-primary course-details-edu' id='$row[ID]'> View Details</button>
                        </div>
                        </div>");
                    }
                    ?>
    ...
    <div class='add_account_cadre_group'>
        <button class='btn btn-warning create-course'>Create New Course</button>
    </div>
