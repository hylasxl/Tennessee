        ...
        <div class="content" style="width: 95% !important; padding-left: 230px !important;">
            ...
            <div class="input-form d-flex ms-5 mt-5">
                <?php
                    ...
                    $data = $conn->query("SELECT max(id) FROM event");
                    $data = $data->fetch_assoc();
                    $data = $data['max(id)'] + 1;
                    echo "<p class=\"me-4 fw-bold \" style='width:150px;'>ID:<span style='color: red;'>*</span> </p>
                    <input autocomplete='off' readonly type=\"text\" style=\"height: 30px; width: 400px; border: none; border-bottom: 1px solid #0d47a1; outline: none; color: #0d47a1;\" placeholder=\"\"   value='$data'>
                </div>";
                ?>
                
            <form action="./php/event_add.php" method='POST'>
                <div class="input-form d-flex ms-5 mt-3">
                    <p class="me-4 fw-bold " style='width:150px;'>Name of Event: </p>
                    <input autocomplete='off' required type="text" style="height: 30px; width: 400px; border: none; border-bottom: 1px solid #0d47a1; outline: none; color: #0d47a1;" placeholder="Type name of the event" onkeypress="this.setAttribute('value',this.value);" value='' name='name' class='name'>
                </div>
                <div class="input-form d-flex ms-5 mt-3">
                    <p class="me-4 fw-bold " style='width:150px;'>DateTime: </p>
                    <input autocomplete='off' required type="text" style="height: 30px; width: 400px; border: none; border-bottom: 1px solid #0d47a1; outline: none; color: #0d47a1;" placeholder="Select time" onchange="this.setAttribute('value',this.value);" id='event-edit-time' name='time' class='datetime' value=''>
                </div>
                <div class="input-form d-flex ms-5 mt-3">
                    <p class="me-4 fw-bold " style='width:150px;'>Duration: </p>
                    <input autocomplete='off' required type="text" style="height: 30px; width: 400px; border: none; border-bottom: 1px solid #0d47a1; outline: none; color: #0d47a1;" placeholder="Duration" onchange="this.setAttribute('value',this.value);" value='' name='duration' class='duration' id='event-edit-duration'>
                </div>
                
                
                <div class='d-flex' style='margin-left: 550px;'>
                    <button class='mt-4 fw-bold event-add-save' type='submit'  id='$data[id]' style=' color: white; background-color: #0d47a1; border: none; outline: none; height: 30px; '>Save</button>
                </div>
            </form>
