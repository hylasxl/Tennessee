...
<body>
    ...
    <div class="main-container">
        <div class="sidebar">
            ...
            <div class='type-3 d-none'>
                ...
                <a href="./lecturer_check_attendance.php" class="field-tag">
                    <div class="field-group">
                        <i class="fa-solid fa-clipboard-check" style="color: #ffc107;"></i>
                        <div class="title-field active-tag">Check Attendance</div>
                    </div>
                </a>
        ...
        <div class="main-content">
            <div class="page-title">Check Attendance</div>
            <div class="row headerrrr d-flex flex-nowrap">
                <div class='col col-2'></div>
                <?
                $classID = $_GET['id'];
                $data = $conn->query("SELECT * FROM class_schedule WHERE classID ='$classID'");
                $daycount = 0;
                while($row = $data->fetch_assoc()){
                    $daycount++;
                    echo "<div class='col col-2 text-center'>$row[date]</div>";
            ...
            <?
                ...
                $classDate = [];
                $datesofClass = $conn->query("SELECT * FROM class_schedule WHERE classID = '$classID'");
                while($row = $datesofClass ->fetch_assoc()){
                    array_push($classDate,$row['date']);
                }
                while($row = $data->fetch_assoc()){
                    ...
                    echo "<div class='row d-flex flex-nowrap' style=''>
                    <div class='col col-2' style='border-right: 2px solid #1a2d59;'>$firstName $lastName</div>
                    ";
                    for($index = 1; $index <= $daycount; $index++){
                        $array_index = $index - 1;
                        $attendance = $conn->query("SELECT * FROM class_attendance where classID = '$classID' and studentID = '$studentID' and date='$classDate[$array_index]' ");
                        $attendance = $attendance->fetch_assoc();
                        $status = $attendance['status'];
                        if($status == 0 || $status == ""){

                            echo "<div class='col col-2 text-center checkboxes'>
                            <input type='checkbox' class='checkbox_attendance' value='/$studentID/$index/$classID' style='width: 20px; height: 20px;'>
                            </div>";
                        } else if($status == 1) {
                            echo "<div class='col col-2 text-center checkboxes'>
                            <input type='checkbox' class='checkbox_attendance' value='/$studentID/$index/$classID' style='width: 20px; height: 20px;' checked>
                            </div>";
                        } else if($status == -1){
                            echo "<div class='col col-2 text-center checkboxes'>
                            <input type='checkbox' class='checkbox_attendance' value='/$studentID/$index/$classID' style='width: 20px; height: 20px;' checked disabled>
                            </div>";
                        }
                    }
                    echo "</div>";
    ...
    <div class='add_account_cadre_group'>
        <button class='btn btn-warning check-submit'>Save Change</button>
    ...
    <script src="./js/class_management.js"></script>ip
</body>
