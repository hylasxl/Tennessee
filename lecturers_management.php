...
<body>
        ...
        <div class="main-content">
            <div class="data-query">
                <div class="row header-table">
                    ...
                    <div class="col col-2">Academic Level</div>
                    <div class="col col-2">No. Assigned Classes</div>
                    <div class="col col-2">Action</div>
                </div>
                <div class="changable-data">
                    <?php
                        
                        $data = $conn->query("SELECT * FROM account where accountType = 3");
                        while($row = $data->fetch_assoc()){
                            ...
                            $language = $conn->query("SELECT * FROM lecturer_language WHERE lecturerID ='$ID'");
                            $language = $language->fetch_assoc();
                            $languageID = $language['languageID'];
                            $languageName = getDataFromTheSameTable("languageName",'language','ID',$languageID,$conn);
                            $academicID = getDataFromTheSameTable("academicLevelID" ,'lecturer_academic_level' ,'lecturerID' ,$ID,$conn);
                            $academicLevel = getDataFromTheSameTable("levelName",'academic_level','ID',$academicID,$conn);
                            $noClasses = $conn->query("SELECT count(ID) from class where taughtBy = '$ID'");;
                            $noClasses = $noClasses->fetch_assoc();
                            $noClasses = $noClasses['count(ID)'];
                            $button_str=" <button id='$ID' class='view-lecturer btn btn-primary'>
                            View Details
                        </button>";
                            ...
                            <div class='col col-2'>$fullName</div>
                            <div class='col col-1'>$gender</div>
                            <div class='col col-2'>$languageName</div>
                            <div class='col col-2'>$academicLevel</div>
                            <div class='col col-2 text-center'>$noClasses</div>
    ...
    <div class='add_account_cadre_group'>
        <button class='btn btn-warning create-teacher'>Send Request To Create New Lecturer Account</button>
    </div>
